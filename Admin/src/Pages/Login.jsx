...
const Login = () => {
  ...
  const onSubmitHandler=async(event)=>{
      ...
      try {
        ...
        console.error("Error during login:", error.response?.data); // This handles the actual error
  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center' >
      <div className=' flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
          ...
          {
          state==="Admin"?<p>Doctor Login? <span onClick={()=>setState('Doctor')}className='text-blue-500 underline cursor-pointer'>Click here</span> </p>:<p>Admin Login? <span onClick={()=>setState('Admin')} className='text-blue-500 underline cursor-pointer'>Click here</span></p>
        }
      </div>
    </form>
